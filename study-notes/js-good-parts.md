...
- 预定义模式：![预定义模式常见模式的简写方式](./images/reg_common_shorthand.png "预定义模式常见模式的简写方式")
...
组匹配和非捕获组匹配的区别：1、使用exec和match方法时会把组匹配放入到exec和match方法返回的匹配数组（匹配分组）中。2、非捕获组匹配性能要比组匹配性能好  
